...
// Порт MPlayer2 (Masha) из проекта CardFool.
// Средняя фабрика: RankGroupAttackStrategy (атака всей группой) + LowestCoverDefendStrategy (экономная защита).
internal class IntermediatePlayer : IPlayer
{
    ...
    private void EnsureInitialized()
    {
        ...
        {
            IStrategyFactory factory = new IntermediateStrategyFactory();
            _attackStrategy = factory.CreateAttackStrategy();
            _defendStrategy = factory.CreateDefendStrategy();
        }
    }

    public string GetName() => "Masha (Intermediate)";
